    ...
    if resposta_login.status_code == 200:
        ...
        if not session_token:
            ...
            if resposta_export.status_code == 200:
                ...
                print(mensagem)
                enviar_email("Backup relógio ponto matriz finalizado com sucesso", mensagem, destinatario)
            else:
                mensagem = f"Erro ao efetuar backup relogio ponto matriz: {resposta_export.status_code}\nResposta do servidor: {resposta_export.text}"
                print(mensagem)
                enviar_email("Erro ao efetuar backup relogio ponto matriz", mensagem, destinatario)
